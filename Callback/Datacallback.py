...
class DepthSensor:
    ...
    def _save_to_file(self, timestamp, x, y, depth, gyro_data, accel_data):
        """บันทึกข้อมูลลงไฟล์แบบ append"""
        try:
            with open(self.save_file, 'a', encoding='utf-8') as f:
                f.write(f"{timestamp}, {x}, {y}, {depth:.4f}, {gyro_data.x:.4f}, {gyro_data.y:.4f}, {gyro_data.z:.4f}, {accel_data.x:.4f}, {accel_data.y:.4f}, {accel_data.z:.4f}\n")
