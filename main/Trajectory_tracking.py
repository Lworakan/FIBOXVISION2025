        ...
        while True:
            ...
            for result in results: 
                ...
                if boxes and len(boxes) > 0:
                    ...
                    # This is the part of your code that needs modification.
                    # Replace your existing RealSense depth processing section with this code:

                    # --- In the main loop where you process detection results ---
                    ...
                    if USE_REALSENSE and depth_frame:
                        # Get raw depth from RealSense
                        raw_realsense_depth = get_realsense_depth(depth_frame, x1, y1, bbox_width, bbox_height)
                        
                        # Store the raw depth value before prediction
                        current_raw_z = raw_realsense_depth if raw_realsense_depth > 0 else z_est
                        
                        # Use raw depth as current_z initially
                        current_z = current_raw_z
                        
                        # Create data for depth prediction
                        depth_pred_data = [
                            [x1, y1, x2, y2, conf, current_z, (x2 - x1) * (y2 - y1)]
                        ]
                        hope = pd.DataFrame(depth_pred_data, 
                                            columns=['X_min', 'Y_min', 'X_max', 'Y_max', 
                                                    'Confidence', 'Average_Depth_m', 'area'])
                        
                        # Get predicted depth from model
                        predicted_z = predict_model(loaded_model, data=hope)['Average_Depth_m'][0]
                        
                        # Now use the predicted depth as the current_z for further calculations
                        current_z = predicted_z
                    else:
                        raw_realsense_depth = 0.0
                        current_raw_z = z_est
                        predicted_z = 0.0
                        current_z = z_est
                    ...
                    # Add to current frame data with both raw and predicted depth values
                    current_frame_data.append([
                        timestamp, frame_count, x1, y1, x2, y2, f"{conf:.2f}", 
                        (x2 - x1) * (y2 - y1), display_rel_x, display_rel_y, f"{current_z:.2f}", 
                        f"{distance_3d:.2f}", f"{raw_realsense_depth:.3f}",
                        f"{rel_x_to_ref:.2f}", f"{rel_y_to_ref:.2f}", f"{rel_z_to_ref:.2f}",
                        f"{fixed_angle:.2f}"
                    ])

                    # --- Then modify the info box section to display both raw and predicted depth ---
                    # Find the section of code that creates the info box and replace it with this:
                    ...
                    if draw_trajectories:
                        ...
                        cv2.rectangle(overlay, 
                                    ...
                                    (info_box_x + 280, info_box_y + 220),  # Increased height to accommodate additional lines
                                    (0, 0, 0), 
                                    -1)
                        ...
                        # Z coordinate - now show both raw and predicted
                        if USE_REALSENSE and raw_realsense_depth > 0:
                            # Show both raw and predicted depth
                            cv2.putText(display_image, f"Z Raw: {current_raw_z:.3f} m", 
                                    (info_box_x + 10, info_box_y + 100), 
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 1)
                            
                            cv2.putText(display_image, f"Z Pred: {predicted_z:.3f} m", 
                                    (info_box_x + 10, info_box_y + 120), 
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,255), 1)
                            
                            if reference_set:
                                cv2.putText(display_image, f"Rel Z: {rel_z_to_ref:+.3f} m", 
                                        (info_box_x + 150, info_box_y + 110), 
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 1)
                        else:
                            cv2.putText(display_image, f"Z Est: {z_est:.2f} m", 
                                    (info_box_x + 10, info_box_y + 100), 
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 1)
                            if reference_set:
                        ...
                        # Adjust the position of the 3D distance display to accommodate the new line
                        ...
                        # Display angles with adjusted positions
