...
try:
    ...
    print("RealSense SDK found - RealSense features enabled")
except ImportError:
    REALSENSE_AVAILABLE = False
    print("RealSense SDK not found - falling back to regular webcam")

# --- Configuration ---
MODEL_PATH = r"./Callback/yolo11l.pt"  # model path
OUTPUT_CSV_FILE = 'tracking_with_realsense.csv'  # output csv file
# -----------------------------
...
USE_REALSENSE = REALSENSE_AVAILABLE  # Set to False to force webcam even if RealSense is available
CAMERA_ID = 0  # Default webcam (change if you have multiple cameras)
FRAME_WIDTH = 640
FRAME_HEIGHT = 480
FPS = 30

# CSV Header with RealSense distance
        ...
        while True:
            ...
            for result in results: 
                ...
                for box in boxes:
                    ...
                    # Draw line from origin to detected object
            ...
            mode_text = "RealSense" if USE_REALSENSE else "Webcam (Depth Estimated)"
            ...
            cv2.putText(display_image, "Press 's': Save Data | 'q': Quit", 
                       (10, instruction_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)

            # Display the frame
            window_title = "Tracking with " + ("RealSense" if USE_REALSENSE else "Webcam")
