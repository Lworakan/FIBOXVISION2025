...
from camera_handler import (
    CameraHandler,
    REALSENSE_AVAILABLE,
)  # Import REALSENSE_AVAILABLE มาด้วย
...
def main_app():
    ...
    print("\n===== Camera Selection =====")
    print(
        "A: Use RealSense camera"
        if REALSENSE_AVAILABLE
        else "A: RealSense not available"
    )
    ...
    while True:
        ...
        if choice == "A" and REALSENSE_AVAILABLE:
            ...
            if choice == "A" and not REALSENSE_AVAILABLE:
                print("RealSense is not available. Select B or C.")
            else:
    ...
    try:
        while True:
            ...
            depth_scale = (
                camera.get_depth_scale() if source_type == "realsense" else 1.0
            )
            ...
            tracking_data = tracker.process_detection(
                target_detection_info, depth_result, origin_x, origin_y
            )
            ...
            if tracking_data["detected"]:
                ...
                vis.draw_detection_results(
                    display_image, detection_vis_data, class_names
                )
                vis.draw_origin_to_target_line(
                    display_image,
                    origin_x,
                    origin_y,
