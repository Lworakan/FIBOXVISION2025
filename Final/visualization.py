...
def draw_coordinate_system(image, orig_x, orig_y):
    ...
    # แกน X (แดง) ชี้ขวา
    cv2.arrowedLine(
        image, (orig_x, orig_y), (orig_x + size, orig_y), (0, 0, 255), thickness
    )
    cv2.putText(
        image, "X", (orig_x + size + 5, orig_y + 10), font, font_scale, (0, 0, 255), 2
    )
    # แกน Y (เขียว) ชี้ขึ้น
    cv2.arrowedLine(
        image, (orig_x, orig_y), (orig_x, orig_y - size), (0, 255, 0), thickness
    )
    cv2.putText(
        image, "Y", (orig_x + 10, orig_y - size - 5), font, font_scale, (0, 255, 0), 2
    )
    # แกน Z (น้ำเงิน) จุด
    cv2.circle(image, (orig_x, orig_y), thickness + 2, (255, 0, 0), -1)
    cv2.circle(image, (orig_x, orig_y), thickness + 7, (255, 0, 0), 1)
    cv2.putText(
        image, "Z", (orig_x - 25, orig_y + 10), font, font_scale, (255, 0, 0), 2
    )
    # ป้าย Origin
    cv2.putText(
        image,
...
def draw_info_panel(image, tracking_data):
    ...
    if tracking_data.get("detected", False):
        ...
        info_lines_data.append(("Real Depth:", f"{z:.2f} m", text_color))
    ...
    if info_lines_data:
        ...
        panel_x = 5
        panel_y = info_text_y_start - panel_height
        ...
        for label, value, color in info_lines_data:
            ...
            cv2.putText(
                image, value, (value_x - 5, y_pos), font, font_scale, color, thickness
            )  # วาด Value ด้วยสีที่กำหนด
            y_pos += line_height
