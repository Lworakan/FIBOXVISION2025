...
class CameraHandler:
    ...
    def _setup_realsense(self):
        ...
        try:
            ...
            print(
                f"Configuring RealSense: {self.width}x{self.height} @ {self.config.FPS} FPS"
            )
            rs_config.enable_stream(
                rs.stream.depth, self.width, self.height, rs.format.z16, self.config.FPS
            )
            rs_config.enable_stream(
                rs.stream.color,
                self.width,
    ...
    def _setup_webcam(self):
        ...
        try:
            ...
            if not self.cap.isOpened():
                print(
                    f"[Warning] Could not open webcam with ID {self.config.CAMERA_ID}, trying default (0)..."
                )
            ...
            print(
                f"Configuring Webcam: {self.width}x{self.height} @ {self.config.FPS} FPS"
            )
            ...
            print(
                f"Webcam actual properties: {actual_w}x{actual_h}, {actual_fps:.2f} FPS"
            )
    ...
    def _setup_video(self):
        ...
        try:
            ...
            last_tried = video_path
            while not os.path.exists(video_path):
                print(f"[Error] Video file not found: {video_path}")
                new_path = input(
                    f"Enter path to video file (ล่าสุด: {last_tried}) หรือกด Enter เพื่อยกเลิก: "
                ).strip()
                if not new_path:
                    print(
                        "[Error] No video file selected. Cancelling video initialization."
                    )
                    return False
                last_tried = new_path
                video_path = new_path
